---
description: "Phase 3: Breadth Analysis Driver -- spawns parallel breadth agents for Soroban audits"
---

# Phase 3: Parallel Breadth Analysis (Soroban)
...
1. Read the skill file from `~/.claude/agents/skills/soroban/{template-name}/SKILL.md` (folder name is lowercase-hyphenated)
...
**Soroban note**: Soroban skills average 30% larger than other languages. Merges that fit at Solana sizes often exceed 300 lines at Soroban sizes. Always verify mechanically.
...
for each planned agent:
  ...
  for each SKILL.md assigned:
    wc -l ~/.claude/agents/skills/soroban/{skill-name}/SKILL.md
