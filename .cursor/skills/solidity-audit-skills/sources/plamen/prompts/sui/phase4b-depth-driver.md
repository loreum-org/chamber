---
description: "Phase 4b: Depth Loop Driver -- spawns depth agents, scanners, niche agents, and support agents for Sui audits"
---
...
Use the Sui Move random-input test support from `rules/language-toolchain-registry.json`
when a stateful invariant can be expressed as `#[random_test]`; otherwise write
boundary-value parameterized tests (handled by depth agents per phase4b-depth-templates.md).
