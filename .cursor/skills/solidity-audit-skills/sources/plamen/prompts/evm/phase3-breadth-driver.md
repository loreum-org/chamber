---
description: "Phase 3: Breadth Analysis Driver -- spawns parallel breadth agents for EVM audits"
---

# Phase 3: Parallel Breadth Analysis (EVM)
...
If `{SCRATCHPAD}/slither/primitive_status.md` exists and contains `SLITHER_PREBAKE_COMPLETE: true`, instruct every breadth agent to include this at the TOP of their analysis prompt, before the analysis task section:
...
- The graph may miss dynamic calls (interfaces, delegatecall, assembly). It is structural, not semantic.
```

If `{SCRATCHPAD}/slither/primitive_status.md` does not exist or does not contain `SLITHER_PREBAKE_COMPLETE: true`, skip this step entirely — agents build their own structural understanding from source (existing behavior).
...
1. Read the skill file from `~/.claude/agents/skills/evm/{template-name}/SKILL.md` (folder name is lowercase-hyphenated)
...
for each planned agent:
  ...
  for each SKILL.md assigned:
    wc -l ~/.claude/agents/skills/evm/{skill-name}/SKILL.md
