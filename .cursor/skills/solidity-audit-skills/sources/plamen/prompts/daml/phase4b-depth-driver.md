---
description: "Phase 4b: Depth Loop Driver -- spawns depth agents, scanners, niche agents, and support agents for DAML/Canton audits"
---

# Phase 4b: Adaptive Depth Loop Driver (DAML)
...
- (a) Combined Authority + Choice/Asset-Flow (depth-state-trace + depth-token-flow lanes)
- (b) Combined Boundary/Invariant + CID-Capability/Interface/Privacy (depth-edge-case + depth-external lanes)
- (c) Combined scanner A+B+C
- (d) Validation sweep

Scanner C CHECK 9 (CID-binding & fail-open/brick — the DAML NEVER-CUT untrusted-target equivalent) MUST survive this merge.
...
- `~/.claude/prompts/daml/phase4b-depth-templates.md` -- depth agent role definitions and methodology
- `~/.claude/prompts/daml/phase4b-scanner-templates.md` -- scanner agent definitions
...
|-------|------|-------|-------------|
| Depth 1 | depth-token-flow (Choice / Asset-Flow) | opus | `depth_token_flow_findings.md` |
| Depth 2 | depth-state-trace (Authority + State/Key-Lifecycle) | opus | `depth_state_trace_findings.md` |
| Depth 3 | depth-edge-case (Boundary / Invariant) | sonnet | `depth_edge_case_findings.md` |
| Depth 4 | depth-external (CID-Capability + Interface + Privacy) | sonnet | `depth_external_findings.md` |
| Scanner A | Choices & Assets | sonnet | `blind_spot_a_findings.md` |
| Scanner B | Authorization, Keys & Interface | sonnet | `blind_spot_b_findings.md` |
| Scanner C | CID-Capability, ensure/abort & Disclosure | sonnet | `blind_spot_c_findings.md` |
| Validation Sweep | Cross-cutting validation | sonnet | `validation_sweep_findings.md` |

**Model diversity rule**: depth-token-flow (Choice / Asset-Flow) and depth-state-trace (Authority + State/Key-Lifecycle) use `model="opus"` (asset-value tracing and authority/state transition analysis require highest reasoning). All others use `model="sonnet"`.
...
Each depth agent reads its role definition from `~/.claude/prompts/daml/phase4b-depth-templates.md` and its full methodology from `~/.claude/agents/depth-{role}.md`.
...
## Depth Methodology
Read: ~/.claude/prompts/daml/phase4b-depth-templates.md -- follow the {ROLE} section
...
- {SCRATCHPAD}/state_variables.md (all (Template,field) state)
- {SCRATCHPAD}/function_list.md (all choices)
- {SCRATCHPAD}/attack_surface.md (attack surface + AUTHORIZATION MATRIX)
...
Each scanner reads its checks from `~/.claude/prompts/daml/phase4b-scanner-templates.md`.

```
You are Blind Spot Scanner {LETTER}: {SCANNER_NAME}.

## Scanner Methodology
Read: ~/.claude/prompts/daml/phase4b-scanner-templates.md -- follow Scanner {LETTER} section
...
## Methodology
Read: ~/.claude/prompts/daml/phase4b-scanner-templates.md -- follow the Validation Sweep section
...
## Your Task
1. Identify all configurable parameters (template config fields, caps, deadlines, fee/rate fields) and their stated/implied valid ranges
2. For each parameter: what happens at the boundary of its valid range?
3. Check constraint coherence: do independently-settable parameters interact? Can valid values of A + valid values of B produce an invalid system state (e.g., an `ensure` clause that becomes unsatisfiable, an accumulator that exceeds a cap across transactions)?
4. Check design limits: what is the maximum issuance / number of active contracts / accumulator total? What invariant breaks first?
...
## THOROUGH CHECKPOINT: Pre-Depth Boundary-Value Campaigns

When `{MODE}` is `thorough`, run this BEFORE spawning depth agents:

### DAML Boundary-Value Script Campaign (no native fuzzer/SAST, zero budget cost)
DAML has no native fuzzer and no security SAST (DLint is style-only). There is no `cargo-fuzz`/`medusa` equivalent to spawn. Instead, the boundary-value campaign is handled INLINE by the depth agents per `~/.claude/prompts/daml/phase4b-depth-templates.md` (Edge Case / Boundary-Invariant lane): each Medium+ boundary finding is exercised via Aptos-style boundary-value parameterized `Script ()` functions (`boundaryMin`/`boundaryMid`/`boundaryMax` calling a shared helper). No separate campaign agent is spawned.

### Assert Completion
```
ASSERT: no separate fuzz artifact is required for DAML (NO_NATIVE_FUZZER_BOUNDARY_SCRIPTS logged)
Boundary-value coverage is asserted via the depth-edge-case agent's depth_edge_case_findings.md output.
```
