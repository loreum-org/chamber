...
contract ChamberSymTest is Test, SymTest {
    ...
    function symbolicSessionKeyCanSubmitUntilTransfer() public {
        ...
        sessionOwner.execute(
            address(chamber),
            abi.encodeCall(IChamber.setDirectorOperator, (uint256(1), sessionKey, type(uint64).max, type(uint32).max))
        );
    ...
    function _submitTx(uint256 tokenId) internal view returns (bytes memory) {
        return abi.encodeWithSignature(
            "submitTransaction(uint256,address,uint256,bytes)", tokenId, TARGET, uint256(0), bytes("")
        );
    }
