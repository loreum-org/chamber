...
function main() {
  console.log('🔄 Syncing ABIs from Foundry output...\n');
  ...
  for (const contract of contracts) {
    ...
    if (abi) {
      ...
      console.log(`✅ ${contract.name}: ${abi.length} ABI entries`);
    } else {
      hasErrors = true;
    }
  }
  ...
  if (hasErrors) {
    console.error('\n⚠️  Some ABIs could not be synced. Make sure to run `forge build` first.');
  }

  if (Object.keys(abis).length > 0) {
    const outputPath = join(repoRoot, 'contracts', 'generated-abis.ts');
    const content = generateAbisFile(abis);
    writeFileSync(outputPath, content);
    console.log(`\n📝 Generated: contracts/generated-abis.ts`);
  }
