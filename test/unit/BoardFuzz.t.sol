...
contract BoardFuzzTest is Test {
            ...
            for (uint256 j = 0; j < i; j++) {
                if (tokenIds[i] == tokenIds[j]) {
                    ...
                    if (tokenIds[i] < type(uint256).max) {
                        ...
                        tokenIds[i] = (i + 1) % type(uint256).max;
                        if (tokenIds[i] == 0) tokenIds[i] = 1;
                    }
                }
            }
        }
    ...
    function testFuzz_GetTop(uint256 count, uint256 numNodes) public {
        ...
        // Verify sorted order (only if we have at least 2 elements)
    ...
    function testFuzz_Quorum(uint256 seats) public {
        ...
        // Create a new board to avoid state from setUp
    ...
    function testFuzz_Invariant_MaxNodes(uint256[20] memory tokenIds, uint256[20] memory amounts) public {
        ...
        for (uint256 i = 0; i < 20; i++) {
            ...
            for (uint256 j = 0; j < i; j++) {
                if (tokenIds[i] == tokenIds[j]) {
                    // Use unchecked to avoid overflow check, but ensure we don't exceed max
                    unchecked {
                        if (tokenIds[i] < type(uint256).max) {
